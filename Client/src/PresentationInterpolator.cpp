...
const game::EntityStateDTO* FindEntity(const game::SnapshotDTO& snapshot,
...
game::Fixed LerpFixed(game::Fixed from, game::Fixed to, std::uint16_t alphaPermille) {
    ...
    return static_cast<game::Fixed>(static_cast<std::int64_t>(from) +
                                    ((delta * alphaPermille) / 1000));
}
...
InterpolatedEntityState ToInterpolatedEntity(const game::EntityStateDTO& target,
