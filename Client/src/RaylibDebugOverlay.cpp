...
void DrawRaylibDebugOverlay(const ClientViewFrame& frame, int viewportHeight)
{
    ...
    DrawText("Client view renders a view model, not raw networking/server state.", 24, 56, 18, LIGHTGRAY);
    DrawText(TextFormat(
                 "Tick: %llu | Entities: %zu | Unacked: %zu | Snapshot: %u",
    ...
    DrawText(TextFormat(
        ...
        LIGHTGRAY);
    DrawText(TextFormat(
                 "Connection: %s | Snapshot ACKs: %zu | Reliable event ACKs: %zu",
