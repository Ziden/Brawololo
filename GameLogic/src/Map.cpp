...
ChunkCoord ChunkForPosition(const MapConfig& config, Fixed x, Fixed y) {
    ...
    const auto pixelX =
        std::clamp(FixedToPixels(x), 0, (config.widthTiles * config.tileSizePixels) - 1);
    const auto pixelY =
        std::clamp(FixedToPixels(y), 0, (config.heightTiles * config.tileSizePixels) - 1);
...
std::vector<ChunkCoord> ChunksInAreaOfInterest(const MapConfig& config, ChunkCoord center) {
    ...
    for (auto y = center.y - config.interestRadiusChunks;
         y <= center.y + config.interestRadiusChunks;
         ++y) {
        for (auto x = center.x - config.interestRadiusChunks;
             x <= center.x + config.interestRadiusChunks;
             ++x) {
