...
std::optional<NetworkEventDTO> ToNetworkEventDTO(NetworkEventId eventId,
                                                 Tick serverTick,
    ...
    if (const auto* spawned = std::get_if<PlayerSpawned>(&event); spawned != nullptr) {
        dto.payload =
            PlayerSpawnedEventDTO{spawned->entityId, spawned->clientId, spawned->x, spawned->y};
        return dto;
    }

    if (const auto* warmup = std::get_if<WeaponWarmupStarted>(&event); warmup != nullptr) {
        dto.payload = WeaponWarmupStartedEventDTO{
            warmup->entityId, warmup->startedAtMs, warmup->completesAtMs, warmup->weaponType};
        return dto;
    }
    ...
    if (const auto* projectile = std::get_if<ProjectileSpawned>(&event); projectile != nullptr) {
        dto.payload = ProjectileSpawnedEventDTO{projectile->projectileId,
                                                projectile->ownerId,
                                                projectile->x,
                                                projectile->y,
    ...
    if (const auto* hit = std::get_if<HitConfirmed>(&event); hit != nullptr) {
        dto.payload = HitConfirmedEventDTO{
            hit->attackerId, hit->targetId, hit->projectileId, hit->serverTimeMs};
        return dto;
    }

    if (const auto* damaged = std::get_if<PlayerDamaged>(&event); damaged != nullptr) {
        dto.payload = PlayerDamagedEventDTO{
            damaged->entityId, damaged->attackerId, damaged->damage, damaged->healthAfter};
        return dto;
    }
    ...
    if (const auto* respawned = std::get_if<PlayerRespawned>(&event); respawned != nullptr) {
        dto.payload = PlayerRespawnedEventDTO{respawned->entityId, respawned->x, respawned->y};
        return dto;
    }

    if (const auto* entered = std::get_if<EntityEnteredInterest>(&event); entered != nullptr) {
        dto.payload = EntityEnteredInterestEventDTO{entered->entityId, entered->observerClientId};
        return dto;
    }

    if (const auto* left = std::get_if<EntityLeftInterest>(&event); left != nullptr) {
        dto.payload = EntityLeftInterestEventDTO{left->entityId, left->observerClientId};
        return dto;
    }
...
std::optional<DomainEvent> ToDomainEvent(const NetworkEventDTO& event) {
    ...
    if (const auto* projectile = std::get_if<ProjectileSpawnedEventDTO>(&event.payload);
        projectile != nullptr) {
        return ProjectileSpawned{projectile->projectileId,
                                 projectile->ownerId,
                                 projectile->x,
                                 projectile->y,
                                 projectile->sourceWeapon};
    ...
    if (const auto* hit = std::get_if<HitConfirmedEventDTO>(&event.payload); hit != nullptr) {
        return HitConfirmed{hit->attackerId, hit->targetId, hit->projectileId, hit->serverTimeMs};
    }
