...
std::optional<SnapshotDTO> SnapshotBaselineCache::Find(ClientId clientId,
    ...
    const auto baseline =
        std::find_if(baselines.begin(), baselines.end(), [snapshotId](const SnapshotDTO& snapshot) {
...
SnapshotBaselineDecision SnapshotBaselineCache::Decide(ClientId clientId,
