...
public:
    ...
    [[nodiscard]] bool ConnectClient(game::ClientId clientId, game::TimestampMs clientTimestampMs = 0);
    [[nodiscard]] bool SubmitInput(const game::ClientInputPacket& packet);
    void Tick();

    [[nodiscard]] game::SnapshotDTO BuildSnapshotFor(game::ClientId clientId, game::SnapshotId baselineId = 0);
    [[nodiscard]] std::optional<game::SnapshotDTO> HistoricalSnapshotAtOrBefore(game::TimestampMs serverTimeMs) const;
    [[nodiscard]] game::LagCompensationResult QueryLagCompensation(
        const game::ClientInputPacket& packet,
