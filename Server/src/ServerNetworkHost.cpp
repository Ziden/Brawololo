...
ServerNetworkHost::ServerNetworkHost(game::net::ITransport& transport, ServerNetworkHostConfig config)
    : transport_(transport)
    , config_(config)
    , baselineCache_(config_.baselines)
{
}

bool ServerNetworkHost::ConnectSimulationOnlyClient(game::ClientId clientId, game::TimestampMs nowMs)
{
...
void ServerNetworkHost::TickAndSendSnapshots(game::TimestampMs nowMs)
{
    ...
    for (const auto clientId : clients_) {
        ...
        if (SendSnapshot(clientId)) {
            MarkSnapshotSent(clientId, nowMs);
        }
    }

    BroadcastReliableEvents(tickEvents);
...
void ServerNetworkHost::HandleLoginEnvelope(const game::NetworkEnvelope& envelope)
{
    ...
    if (std::find(clients_.begin(), clients_.end(), login->header.clientId) == clients_.end()) {
        ...
        clientStateById_.emplace(
            login->header.clientId,
...
void ServerNetworkHost::HandleNetworkEventAckEnvelope(const game::NetworkEnvelope& envelope)
{
    ...
    const auto state = clientStateById_.find(ack->clientId);
    if (state == clientStateById_.end() ||
        !state->second.AcknowledgeReliableEvent(ack->eventId)) {
...
void ServerNetworkHost::BroadcastReliableEvents(const game::EventList& events)
{
    for (const auto& event : events) {
        ...
        if (const auto* entered = std::get_if<game::EntityEnteredInterestEventDTO>(&networkEvent->payload);
            entered != nullptr) {
            observerOnly = entered->observerClientId;
        } else if (const auto* left = std::get_if<game::EntityLeftInterestEventDTO>(&networkEvent->payload);
            left != nullptr) {
        ...
        for (const auto clientId : clients_) {
            ...
            SendReliableEvent(clientId, *networkEvent);
        }
    }
}

void ServerNetworkHost::SendReliableEvent(game::ClientId clientId, const game::NetworkEventDTO& event)
{
    (void)clientId;
    ...
    if (transport_.Send(envelope)) {
        auto state = clientStateById_.find(clientId);
        if (state != clientStateById_.end()) {
            state->second.TrackReliableEvent(event, event.serverTimeMs);
        }
        ++stats_.reliableEventsSent;
    }
}

bool ServerNetworkHost::SendSnapshot(game::ClientId clientId)
{
    auto state = clientStateById_.find(clientId);
    const auto requestedBaselineId = state == clientStateById_.end()
        ? game::SnapshotId{}
        : state->second.AckedSnapshot();
