...
game::SnapshotDTO ServerRuntime::BuildSnapshotFor(game::ClientId clientId, game::SnapshotId baselineId)
{
    return simulation_.BuildSnapshot(clientId, nextSnapshotId_++, baselineId);
}

std::optional<game::SnapshotDTO> ServerRuntime::HistoricalSnapshotAtOrBefore(game::TimestampMs serverTimeMs) const
{
    return history_.ClosestAtOrBefore(serverTimeMs);
}

game::LagCompensationResult ServerRuntime::QueryLagCompensation(
    const game::ClientInputPacket& packet,
    std::int64_t clientToServerOffsetMs) const
{
    return lagCompensation_.Query(
        history_,
        game::LagCompensationQuery{
