...
void TestSnapshotsAndSerialization()
{
    ...
    Expect(!DeserializeClientInputPacket(std::span<const std::byte>{trailingInputBytes.data(), trailingInputBytes.size()}).has_value(),
        "client input rejects trailing bytes");

    auto invalidFireBytes = inputBytes;
    invalidFireBytes.back() = std::byte{2};
    Expect(!DeserializeClientInputPacket(std::span<const std::byte>{invalidFireBytes.data(), invalidFireBytes.size()}).has_value(),
    ...
    Expect(!DeserializeSnapshot(std::span<const std::byte>{trailingSnapshotBytes.data(), trailingSnapshotBytes.size()}).has_value(),
    ...
    Expect(!DeserializeSnapshot(std::span<const std::byte>{invalidDeliveryKindBytes.data(), invalidDeliveryKindBytes.size()}).has_value(),
